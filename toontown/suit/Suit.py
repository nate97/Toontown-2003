...
class Suit(Avatar.Avatar):
    ...
    def delete(self):
        ...
        try:
            ...
            if self.shadowNull:
                self.shadowNull.removeNode()
                self.shadowNull = None
            
            if self.nametagNull:
                self.nametagNull.removeNode()
                self.nametagNull = None
    ...
    def setSuitClothes(self, modelRoot = None):
        ...
        self.shadowNull = self.find('**/joint-shadow')
    ...
    def playDialogue(self, type, length):
        if len(SuitDialogArray) == 0:
            self.notify.warning('dialog array empty')
            return None
        ...
        if type == 'statementA' or type == 'statementB':
            if length == 1:
                if dialogueArray[0] != None:
                    base.playSfx(dialogueArray[0], node = self)
                
            
            if length == 2:
                if dialogueArray[1] != None:
                    base.playSfx(dialogueArray[1], node = self)
                
            
            if length >= 3:
                if dialogueArray[2] != None:
                    base.playSfx(dialogueArray[2], node = self)
                
            
        elif type == 'question':
            if dialogueArray[3] != None:
                base.playSfx(dialogueArray[3], node = self)
