...
global DogDialogueArray, CatDialogueArray, HorseDialogueArray, RabbitDialogueArray, MouseDialogueArray, DuckDialogueArray, Preloaded
import Avatar
import AvatarDNA
import Actor
import string
...
def loadModels():
    ...
    if preloadAvatars:
        ...
        for bottom in AvatarDNA.GirlBottoms:
            tex = loader.loadTexture(bottom[0])
            tex.setMinfilter(Texture.FTLinearMipmapLinear)
            tex.setMagfilter(Texture.FTLinear)
            Preloaded.append(tex)
...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.animFSM = FSM.FSM('Toon', [
                ...
                State.State('Emote', self.enterEmote, self.exitEmote)], 'off', 'off')
    ...
    def setSpeed(self, forwardSpeed, rotateSpeed):
        ...
        if self.standWalkRunReverse != None:
            ...
            try:
                id = toonbase.tcr.playGame.getPlaceId()
                ...
                if 1:
                    ...
                    showWake = 1
                elif id == ToontownGlobals.MyEstate:
    ...
    def enterTeleportOut(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        self.track = Parallel(holeTrack, name = trackName, autoFinish = 1)
        for hole in holes:
    ...
    def _Toon__doHeadScale(self, scale, lerpTime):
        if scale == None:
            scale = toonHeadScales[self.style.getAnimal()]
