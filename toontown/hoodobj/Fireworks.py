...
def getTexture(textureIndex):
    return loader.loadModelOnce(textures.get(textureIndex))


def shootFirework(style, x, y, z, colorIndex1, colorIndex2, amp):
...
def shootFireworkRing(x, y, z, color1, color2, amp):
    ...
    force0 = LinearSourceForce(Point3(x, y, z + 10), LinearDistanceForce.FTONEOVERR, 0.10000000000000001, 1.1000000000000001 * amp, 1)
    force0.setActive(1)
    f0.addForce(force0)
    force1 = LinearSinkForce(Point3(x, y, z + 10), LinearDistanceForce.FTONEOVERR, 0.5, 2.0 * amp, 1)
    force1.setActive(1)
    f0.addForce(force1)
    f.addForceGroup(f0)
