...
class FriendsListPanel(DirectFrame, StateData.StateData):
    ...
    def load(self):
        ...
        self.secrets = DirectButton(parent = self, relief = None, pos = (0.152, 0.0, 0.14000000000000001), image = (auxGui.find('**/ChtBx_ChtBtn_UP'), auxGui.find('**/ChtBx_ChtBtn_DN'), auxGui.find('**/ChtBx_ChtBtn_RLVR')), text = ('', Localizer.FriendsListPanelSecrets, Localizer.FriendsListPanelSecrets), text_scale = 0.044999999999999998, text_fg = (0, 0, 0, 1), text_bg = (1, 1, 1, 1), text_pos = (-0.040000000000000001, -0.085000000000000006), textMayChange = 0, command = self._FriendsListPanel__secrets)
        gui.removeNode()
        auxGui.removeNode()
